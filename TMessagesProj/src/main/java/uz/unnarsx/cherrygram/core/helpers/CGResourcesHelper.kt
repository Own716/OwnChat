...
object CGResourcesHelper {
    ...
    fun getCodeVersion() : String {
        return BuildConfig.BUILD_CODE_VERSION
    }

    @JvmStatic
    fun getAboutString(): String {
        return getAppName() + " v" + getCherryVersion() + " 正式版 (" + getAbiCode() + ")" +
                    "\n" +
                    Constants.CG_AUTHOR +
                    "\n" +
    ...
    fun getReplyIconDrawable(): Int {
        return when (CherrygramMessagesConfig.messageSlideAction) {
            ...
            CherrygramMessagesConfig.MESSAGE_SLIDE_ACTION_TRANSLATE -> R.drawable.msg_translate_mini_filled_solar
            CherrygramMessagesConfig.MESSAGE_SLIDE_ACTION_TRANSLATE_GEMINI -> R.drawable.msg_translate_mini_filled_solar
